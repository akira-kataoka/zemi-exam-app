  ...
  function renderSessionPopoverList() {
    ...
    wrap.innerHTML = sessions.length ? sessions.map(s => {
      ...
      return `<div class="session-popover-item ${isCurrent ? 'current' : ''}" data-id="${s.id}">
        <button type="button" class="sp-pick" data-pick="${s.id}" title="この試験回に切り替え">
          ${isCurrent ? '●' : '○'}
          <span class="sp-name">${escapeHtml(s.name)}</span>
          <span class="sp-meta">${count}名 / ${dateInfo}${locInfo}</span>
        </button>
        <button type="button" class="sp-rename icon-btn btn" data-rename="${s.id}" title="名称変更" aria-label="${escapeHtml(s.name)} の名称を変更">✏</button>
      </div>`;
    ...
    tbody.innerHTML = _enriched.map(({ c, ac, sv, lastUpdate }) => {
      ...
      <tr data-id="${c.id}" class="${c.passed ? 'row-passed' : ''}">
        ...
        <td class="row-actions">
          <button type="button" class="btn btn-icon" data-act="view" title="詳細を見る" aria-label="詳細を見る">👁</button>
          <button type="button" class="btn btn-icon danger" data-act="del" title="削除" aria-label="削除">🗑</button>
        </td>
      </tr>`;
    }).join('') || (list.length === 0
              ...
              ${records.map(r => `
                <div class="iv-record-mgr-row">
                  ...
                  <div style="display:flex;gap:6px">
                    <button type="button" class="btn btn-icon" data-edit-rec="${r.id}" title="編集" aria-label="${escapeHtml(r.interviewer || '面接')}記録を編集">✏</button>
                    <button type="button" class="btn btn-icon danger" data-del-rec="${r.id}" title="削除" aria-label="${escapeHtml(r.interviewer || '面接')}記録を削除">🗑</button>
                  </div>
                </div>
              `).join('')}
            </div>
